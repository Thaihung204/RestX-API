...
{
    private readonly RestxAdminContext _adminContext;

    public AdminSnapshotService(RestxAdminContext adminContext)
    {
    ...
    public async Task TakeDailySnapshotAsync(DateOnly? forDate = null)
    {
        ...
        var tenants = await _adminContext.Tenants
            .Where(t => t.Status)
            .ToListAsync();
        ...
        foreach (var tenant in tenants)
        {
            ...
            _adminContext.TenantSnapshots.Add(snapshot);
        }
    ...
    public async Task SeedMonthSnapshotsAsync(DateOnly month)
    {
        ...
        var existingSnapshots = _adminContext.TenantSnapshots
            .Where(s => s.PeriodType == "daily"
                     && s.PeriodStart >= firstDay
                     && s.PeriodStart <= lastDay);
        _adminContext.TenantSnapshots.RemoveRange(existingSnapshots);
        await _adminContext.SaveChangesAsync();
    ...
    public async Task TakeMonthlySnapshotAsync(DateOnly? forMonth = null)
    {
        ...
        {
            var dailySnapshots = await _adminContext.TenantSnapshots
    ...
    public async Task<List<TenantSnapshotDto>> GetSnapshotsAsync(string periodType, DateOnly? fromDate = null, DateOnly? toDate = null, Guid? tenantId = null)
    {
        ...
        var query = _adminContext.TenantSnapshots
        ...
        return snapshots
            ...
            .Select(g => new TenantSnapshotDto
            {
                TenantId = g.Key,
                ...
                CompletedOrders = g.Sum(s => s.CompletedOrders),
                CancelledOrders = g.Sum(s => s.CancelledOrders),
                TotalCustomers = g.OrderByDescending(s => s.PeriodStart).First().TotalCustomers,
                NewCustomers = g.Sum(s => s.NewCustomers),
                NewReservations = g.Sum(s => s.NewReservations)
            })
            .ToList();
    }
